...
3. **Inference layer.** Ollama — выбранный runtime. Он уже присутствует вне репозитория и слушает только `127.0.0.1:11434`; TASK-003 подтвердит и оформит его проектную конфигурацию без переустановки. Его OpenAI-совместимый API отделяет UI и агента от конкретной модели.
