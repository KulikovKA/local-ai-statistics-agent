...
Целевая система разделяет пользовательский интерфейс, слой агента, inference, модель и среду выполнения. В TASK-002 выбран inference runtime Ollama; модель и VS Code AI-интеграция ещё не выбраны и не установлены.
...
3. **Inference layer.** Ollama — выбранный runtime. Он уже присутствует вне репозитория и подтверждён на `127.0.0.1:11434`: `/api/version`, `/api/tags` и OpenAI-совместимый `/v1/models` отвечают без модели. Project-owned launchers не меняют существующий процесс и запускают новый процесс только с loopback и `OLLAMA_NO_CLOUD=1`. Его OpenAI-совместимый API отделяет UI и агента от конкретной модели.
...
Причины выбора Ollama, сравнение с `llama.cpp` и vLLM, а также ограничения Radeon 780M описаны в [ADR-001](adr/ADR-001-runtime-selection.md). Подтверждение существующей установки, выбор модели и benchmark остаются будущими отдельными задачами.
